...
export function showRoomView(roomInfo: MessageRoom) {
    ...
    colorPickerDiv.style.backgroundColor = currentPlayer.color.toString('hex');
    colorPicker.value = currentPlayer.color.toString('hex');
    let colorPickerLabel = document.getElementById('color-label');

    let newColor = new TinyColor(colorPicker.value);
    colorPickerLabel.style.color = newColor.isLight() ? newColor.clone().darken(20).toString('hex'): newColor.clone().lighten(20).toString('hex');
...
function enforceMinMax(el: HTMLInputElement) {
  ...
  function handleChangeSettings(el: HTMLInputElement) {
    enforceMinMax(el);
